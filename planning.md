[x] - create user account and model
[x] - use clerk for it
[x] - create sign up
[x] - create sign in
[x] - allow user to login
[_] - make login with google instead of email+password
[x] - create backend with middleware that authenticate the user
[x] - use drizzleORM as the database orm
[x] - create database table for habits collection (user_id, title, description, streak_count, last_completed, frequency, created_at)
[x] - build get habits route in the backend
[x] - build streak functionality.
[_] - make confirmation for deleting a habit or a soft delete instead
[_] - migrate app to only local without a backend
[x] - route complete habit, disallow completing twice a period.
[_] - route completed today habits, if we have lst completed, we can just check in the frontend instead, order_by lst completed.
[x] - streak cnt should restart if the lst completed gap is two time units wider  
[x] - when we create a habit, we have to create with an old date so the user can complete it for the first time

following at: 2:24
